...
public class FileManager extends MinecraftInstance {
    ...
    public final File dir = new File(mc.getDataDir(), Terra.CLIENT_NAME + "-" + Backend.MINECRAFT_VERSION_MAJOR + "." + Backend.MINECRAFT_VERSION_MINOR);
    ...
    private void saveConfig(final FileConfig config, final boolean ignoreStarting) {
        if (!ignoreStarting && Terra.INSTANCE.isStarting())
            return;
    ...
    public void loadBackground() {
        if(backgroundFile.exists()) {
            try {
                ...
                Terra.INSTANCE.setBackground(classProvider.createResourceLocation(Terra.CLIENT_NAME.toLowerCase() + "/background.png"));
                mc.getTextureManager().loadTexture(Terra.INSTANCE.getBackground(), classProvider.createDynamicTexture(bufferedImage));

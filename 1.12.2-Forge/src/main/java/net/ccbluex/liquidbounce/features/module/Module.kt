...
open class Module : MinecraftInstance(), Listenable {
    ...
    var keyBind = Keyboard.CHAR_NONE
        set(keyBind) {
            ...
            if (!Terra.isStarting)
                Terra.fileManager.saveConfig(Terra.fileManager.modulesConfig)
        }
    var array = true
        set(array) {
            field = array
    ...
    var state = false
        set(value) {
            ...
            if (!Terra.isStarting) {
                when (Terra.moduleManager.toggleSoundMode) {
                    1 -> (if (value) Terra.tipSoundManager.enableSoundTerra else Terra.tipSoundManager.disableSoundTerra).asyncPlay()
                    2 -> (if (value) Terra.tipSoundManager.enableSoundRise else Terra.tipSoundManager.disableSoundRise).asyncPlay()
                }

                Terra.hud.addNotification(Notification("Notification", "${if (value) "Enabled " else "Disabled "}$name", if (value) NotifyType.SUCCESS else NotifyType.ERROR))
            }

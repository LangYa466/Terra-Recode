...
@ModuleInfo(name = "Crosshair", description = "Custom CrossHairs.", category = ModuleCategory.RENDER)
class Crosshair : Module() {
    // Color
    private val colorModeValue = ListValue("Color", arrayOf("Custom", "Slowly", "Rainbow"), "Custom")
    private val colorRedValue = IntegerValue("Red", 255, 0, 255).displayable { colorModeValue.get().equals("Custom") }
    private val colorGreenValue = IntegerValue("Green", 255, 0, 255).displayable { colorModeValue.get().equals("Custom") }
    private val colorBlueValue = IntegerValue("Blue", 255, 0, 255).displayable { colorModeValue.get().equals("Custom") }
    ...
    private val widthValue = FloatValue("Width", 0.25f, 0.1f, 10f)
    private val sizeValue = FloatValue("Length", 5f, 0.25f, 15f)
    private val gapValue = FloatValue("Gap", 0.5f, 0.25f, 15f)
    ...
    fun onRender2D(event: Render2DEvent) {
        ...
        /* ?: RaycastUtils.raycastEntity(Reach.hitReach.toDouble()) {
        ...
        if (hitMarkerValue.get() && target != null && target.hurtTime > 0) {
            ...
            GL11.glColor4f(1f, 1f, 1f, target.hurtTime.toFloat() / 10F)

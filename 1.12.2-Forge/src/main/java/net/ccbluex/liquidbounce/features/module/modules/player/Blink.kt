...
class Blink : Module() {
    ...
    private val hytNewValue = BoolValue("CancelC0F", true)
    ...
    fun onPacket(event: PacketEvent) {
        ...
        if (classProvider.isCPacketPlayerPosition(packet) || classProvider.isCPacketPlayerPosLook(packet) ||
                ...
                classProvider.isCPacketEntityAction(packet) || classProvider.isCPacketUseEntity(packet) || (hytNewValue.get() && classProvider.isCPacketConfirmTransaction(
                        packet
                ))
        ) {
            event.cancelEvent()
    ...
    fun onRender3D(event: Render3DEvent?) {
        val breadcrumbs = Terra.moduleManager.getModule(Breadcrumbs::class.java) as Breadcrumbs?

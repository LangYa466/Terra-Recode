...
object ChestAura : Module() {
    ...
    private val delayValue = IntegerValue("Delay", 250, 50, 500)
    ...
    //private val furnaceValue = BlockValue("Furnace", functions.getIdFromBlock(classProvider.getBlockEnum(BlockType.FURNACE)))
    ...
    fun onMotion(event: MotionEvent) {
        if (Terra.moduleManager[Blink::class.java].state || Terra.moduleManager[KillAura::class.java].state) {
            return
        }
        ...
        when (event.eventState) {
            EventState.PRE -> {
                ...
                currentBlock = BlockUtils.searchBlocks(radius.toInt())
                    .filter {
                        (functions.getIdFromBlock(it.value) == chestValue.get()
                                //|| functions.getIdFromBlock(it.value) == furnaceValue.get()
                        )
                                && !clickedBlocks.contains(it.key)

...
class GuiBackground(val prevGui: IGuiScreen) : WrappedGuiScreen() {
    ...
    override fun actionPerformed(button: IGuiButton) {
        when (button.id) {
            ...
            3 -> {
                ...
                try {
                    Files.copy(file.toPath(), FileOutputStream(Terra.fileManager.backgroundFile))

                    val image = ImageIO.read(FileInputStream(Terra.fileManager.backgroundFile))
                    val location = classProvider.createResourceLocation(Terra.CLIENT_NAME.toLowerCase() + "/background.png")

                    Terra.background = location
                    ...
                    Terra.fileManager.backgroundFile.delete()
                }
            }
            4 -> {
                Terra.background = null
                Terra.fileManager.backgroundFile.delete()
            }
    ...
    override fun drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float) {
        ...
        Fonts.font180.drawCenteredString("Background", this.representedScreen.width / 2F, representedScreen.height / 8F + 5F,
                4673984, true)

...
class ScoreboardElement(x: Double = 5.0, y: Double = 0.0, scale: Float = 1F, side: Side = Side(Side.Horizontal.RIGHT, Side.Vertical.MIDDLE)) : Element(x, y, scale, side) {
    ...
    private val backgroundColorAlphaValue = IntegerValue("Background-Alpha", 40, 0, 255)
    private val backGroundShadow = BoolValue("BackgroundShadow", true)
...
//背景
        ...
        scoreCollection.forEachIndexed { index, score ->
            ...
            val hud = Terra.moduleManager.getModule(HUD::class.java)
            if (enableServerValue.get()) {
                if (!serverValue.equals("none")) {
                    for (domain in allowedDomains) {
                        if (name.contains(domain, true)) {
                            name = when (serverValue.get().toLowerCase()) {
                                "clientname" -> HUD.customClientName.get()
                                "hyt" -> "  §c✿ §b§l挂与停 §c✿"
                                else -> "§g${hud.waterMarkValue.get()}"
                            }

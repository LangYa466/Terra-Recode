...
public class GuiAltManager extends WrappedGuiScreen {
    ...
    public static void loadGenerators() {
        try {
            ...
            final JsonElement jsonElement = new JsonParser().parse(HttpUtils.get(Terra.CLIENT_CLOUD + "/generators.json"));
    ...
    public static String login(final MinecraftAccount minecraftAccount) {
        ...
        if (result == LoginUtils.LoginResult.LOGGED) {
            ...
            Terra.fileManager.saveConfig(Terra.fileManager.accountsConfig);
    ...
    public void initGui() {
        ...
        for (int i = 0; i < Terra.fileManager.accountsConfig.getAccounts().size(); i++) {
            MinecraftAccount minecraftAccount = Terra.fileManager.accountsConfig.getAccounts().get(i);
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        Fonts.font35.drawCenteredString(this.searchField.getText().isEmpty() ? (Terra.fileManager.accountsConfig.getAccounts().size() + " Alts") : this.altsList.accounts.size() + " Search Results", representedScreen.getWidth() / 2.0f, 18, 0xffffff);
        ...
        switch (button.getId()) {
            ...
            case 2:
                if (altsList.getSelectedSlot() != -1 && altsList.getSelectedSlot() < altsList.getSize()) {
                    Terra.fileManager.accountsConfig.removeAccount(this.altsList.accounts.get(altsList.getSelectedSlot()));
                    Terra.fileManager.saveConfig(Terra.fileManager.accountsConfig);
            ...
            case 7:
                ...
                while ((line = bufferedReader.readLine()) != null) {
                    ...
                    if (!Terra.fileManager.accountsConfig.accountExists(accountData[0])) {
                        if (accountData.length > 1)
                            Terra.fileManager.accountsConfig.addAccount(accountData[0], accountData[1]);
                        else
                            Terra.fileManager.accountsConfig.addAccount(accountData[0]);
                    }
                }
            ...
            case 11:
                ...
                break;
            case 12:
                if (Terra.fileManager.accountsConfig.getAccounts().size() == 0) {
                    status = "§cThe list is empty.";
                    return;
                }
                ...
                try {
                    ...
                    for (MinecraftAccount account : Terra.fileManager.accountsConfig.getAccounts()) {
    ...
    private class GuiList extends WrappedGuiSlot {
        ...
        private void updateAccounts(String search) {
            if (search == null || search.isEmpty()) {
                this.accounts = Terra.fileManager.accountsConfig.getAccounts();
                return;
            }

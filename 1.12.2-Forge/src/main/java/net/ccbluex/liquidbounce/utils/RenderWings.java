...
public class RenderWings extends ModelBase {
    final Wings Wings = Terra.moduleManager.getModule(Wings.class);
    ...
    public RenderWings() {
        if (WingMode.equals("Dragon")) {
            this.location = new ResourceLocation("terra/wings/DragonWings.png");
        } else if (WingMode.equals("Simple")) {
            this.location = new ResourceLocation("terra/wings/NeonWings.png");
        }
    ...
    public void renderWings(float partialTicks) {
        double rotate;
        if (RotationUtils.targetRotation != null && Rotations.INSTANCE.getBodyValue().get()) {
        ...
        if (Wings.getColourType().get().equals("Chroma")) {
            RenderUtils.glColor(ColorUtils.rainbow(), 255F);
        } else if (Wings.getColourType().get().equals("Custom")) {
            RenderUtils.glColor(new Color(Wings.getCR().get(), Wings.getCG().get(), Wings.getCB().get()), 255F);
        } else {

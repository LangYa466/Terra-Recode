...
public final class EntityUtils extends MinecraftInstance {
            ...
            if (targetInvisible || !entity.isInvisible()) {
                if (targetPlayer && classProvider.isEntityPlayer(entity)) {
                    ...
                    if (canAttackCheck) {
                        ...
                        if (isFriend(entityPlayer) && !Terra.moduleManager.getModule(NoFriends.class).getState())
                            return false;

                        if (entityPlayer.isSpectator())
                            return false;

                        final Teams teams = (Teams) Terra.moduleManager.getModule(Teams.class);
    ...
    public static boolean isFriend(final IEntity entity) {
        return classProvider.isEntityPlayer(entity) && entity.getName() != null &&
                Terra.fileManager.friendsConfig.isFriend(ColorUtils.stripColor(entity.getName()));
    }

    public static boolean isAnimal(final IEntity entity) {
        return classProvider.isEntityAnimal(entity) || classProvider.isEntitySquid(entity) || classProvider.isEntityGolem(entity) || classProvider.isEntityVillager(entity) ||

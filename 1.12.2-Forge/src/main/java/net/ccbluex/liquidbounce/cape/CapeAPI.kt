...
object CapeAPI : MinecraftInstance() {
    ...
    fun registerCapeService() {
        ...
        val jsonObject = JsonParser()
                .parse(HttpUtils.get("${Terra.CLIENT_CLOUD}/capes.json")).asJsonObject
    ...
    fun loadCape(uuid: UUID): CapeInfo? {
        ...
        // Load cape
        val resourceLocation = Terra.wrapper.classProvider.createResourceLocation("capes/%s.png".format(uuid.toString()))
        val capeInfo = CapeInfo(resourceLocation)
        val threadDownloadImageData = Terra.wrapper.classProvider.createThreadDownloadImageData(null, url, null, object : WIImageBuffer {

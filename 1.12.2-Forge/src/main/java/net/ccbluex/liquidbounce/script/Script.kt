...
class Script(val scriptFile: File) : MinecraftInstance() {
    ...
    init {
        ...
        scriptEngine.put("moduleManager", Terra.moduleManager)
        scriptEngine.put("commandManager", Terra.commandManager)
        scriptEngine.put("scriptManager", Terra.scriptManager)
    ...
    fun registerModule(moduleObject: JSObject, callback: JSObject) {
        ...
        Terra.moduleManager.registerModule(module)
    ...
    fun registerCommand(commandObject: JSObject, callback: JSObject) {
        ...
        Terra.commandManager.registerCommand(command)
    ...
    private fun supportLegacyScripts() {
        if (getMagicComment("api_version") != "2") {
            ...
            val legacyScript = Terra::class.java.getResource("/assets/minecraft/terra/scriptapi/legacy.js").readText()
    ...
    fun onDisable() {
        ...
        registeredModules.forEach { Terra.moduleManager.unregisterModule(it) }
        registeredCommands.forEach { Terra.commandManager.unregisterCommand(it) }
    ...
    fun import(scriptFile: String) {
        scriptEngine.eval(File(Terra.scriptManager.scriptsFolder, scriptFile).readText())
    }
